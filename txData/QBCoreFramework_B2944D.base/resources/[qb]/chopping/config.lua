...
----------
----------
Config.Phone = "qb" -- (quasar) (renewed) (qb) Whatever phone script you are using for email (email function is in open.lua and called ChoppingEmail()).
...
Config.DispatchScript = "ps-dispatch" -- This is for the police call function in open.lua CallPoliceForCarJack()
...
Config.ClothingMenuEvent = "qb-clothing:client:openMenu" -- (ox_appearance:wardrobe) MAME THIS NIL IF YOU DONT WANT CLOTHING ROOM TO BE AN UPGEADE event used to give players the clothing menu when they upgrade there garage to have this option
----------
----------
...
Config.RareItem2 = "nitrous" -- Another very rare item you cab only get when searching a trunk if your garage has earned over Config.CarsNeededToEarnForDrop cars overall
...
Config.RareItem1 = "boostinglaptop" -- A very rare item you cab only get when searching a trunk if your garage has earned over Config.CarsNeededToEarnForDrop cars overall
...
Config.GetCopCarChanceIfLandsOnCopCar = 30 -- % chance of getting the police car if the random roll of a vehicle lands on a cop car (Ignore this if Config.UseSharedVehicleCars is false as this never gets used if Config.UseSharedVehicleCars is false)
----------
----------
...
Config.MaximumMembers = 5 -- How many members can be apart of one garage
...
----------
Config.CoolDown = 7 -- How long people must wait before starting chop breakdown of a car (in minutes)
----------
...
Config.FrameworkFunctions = {
    ...
    GetPlayer = function(source,cid,client)
        if Framework == 'QBCore' then
            ...
            local player = nil
            if cid then
                self.PlayerData = {source = PugFindPlayersByItentifier(source) }
                return self
            elseif client then
