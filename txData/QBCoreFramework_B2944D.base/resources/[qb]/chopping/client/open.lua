...
RegisterNetEvent("Pug:client:AddClothesTarget", function()
	if CurrentGarage then
		if Config.Target == "ox_target" then
			AddClothesTarget = exports.ox_target:addBoxZone({
...
RegisterNetEvent("Pug:client:ChopDownVehiclePart", function(Data)
	...
	if HasItem(Data.item, 1) then
		if not ChoppingCarPart then
			...
			local Item1 = CreateObject(GetHashKey(Data.prop), 0, 0, 0, true, true, true)        
			...
			if #(GetEntityCoords(Ped) - vector3(995.2, -2988.37, -40.65)) >= 1.1 then
				HadToWalk = true
			...
			Item2 = CreateObject(GetHashKey(Data.prop), Data.vec3, true, true, true)
