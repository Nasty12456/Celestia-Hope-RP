        ...
        options = {
            {
                ...
                action = function()
                    TriggerEvent('qb-oxyruns:initiate')
                end,
            },
        },
        distance = 2.0
...
RegisterNetEvent('qb-oxyruns:initiate', function ()
    if CurrentCops >= Config.CopsNeeded then
        ...
        QBCore.Functions.Progressbar("start_delivery", 'Getting Information', 10000, false, true, {
            ...
            if Config.UseStartPayment == 'true' then
                QBCore.Functions.TriggerCallback("qb-oxyruns:Haspayment",function(money)
                    if money then
                        TriggerServerEvent("qb-oxyruns:coolout")
                        TriggerEvent('qb-oxyruns:OxyStart')
                    end
                end)
...
RegisterNetEvent('qb-oxyruns:OxyStart', function ()
    if Config.Notifications == 'phone' then
        if Config.Phone == 'qb' then
...
function Collect()
    ...
    TriggerServerEvent('qb-oxyruns:giveoxypackages')
...
function GivePackage()
    TriggerServerEvent('qb-oxyruns:removeoxypackages')
...
RegisterNetEvent('qb-oxyruns:clean', function ()
...
CreateThread(function()
    while true do
        if OnDelivery then
            QBCore.Functions.TriggerCallback('qb-oxyruns:hasPackage', function(Package)
