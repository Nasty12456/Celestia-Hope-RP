...
$(document).on('click', '.fa-shopping-cart', function (e) {
  ...
  if (existingCartItem) {
    ...
    $.post("http://qb-shops/setCart", JSON.stringify({
      type: type,
      price: price,
      image: image,
      id: id,
...
$(document).on('click', '.pay', function (e) {
  ...
  $.post("http://qb-shops/pay", JSON.stringify({price:totalCartPrice,payType: payType, cartArr: cartArr}));
})
...
function closeNUI() {
  $("body").hide()
  $.post("http://qb-shops/close", JSON.stringify({}));
  $(".job-box,.job-cart-circle").animate({
    top: "15rem"
  }, 500)
}
