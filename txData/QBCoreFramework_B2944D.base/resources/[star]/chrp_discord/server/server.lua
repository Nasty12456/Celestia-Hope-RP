...
RegisterServerEvent('chrp_discord:removeFromQueue')
AddEventHandler('chrp_discord:removeFromQueue', function()
...
lib.callback.register('chrp_discord:getConfig', function(source)
    local cConfig = copyTable(Config)
    cConfig.DiscordInfo = nil 
    return cConfig
end)

lib.callback.register('chrp_discord:checkForRole', function(source, role)
    local hasRole = checkRole(source, role)
    return hasRole
end)

lib.callback.register('chrp_discord:getRoles', function(source, role)
