...
Citizen.CreateThread(function()
  while true do
      ...
      if dist <= 5.0 then
          sleep = 1
          if dist <= 3.0 then
              ShowHelpNotification('Press ~INPUT_CONTEXT~ to Start Illegal activity')
...
function CheckStartRob()
  QBCore.Functions.TriggerCallback('pp-weedheist:server:checkTime', function(time)
      if time then
          TacoTruckBlip = addBlip(Config['Rob']['TruckSpawnCfg'], 67, 1, 'Truck Locaion')
          QBCore.Functions.Notify('Weed Heist Started. Check Truck Locaion in Map')
          SpawnGuardsandTruck()
...
function SpawnGuardsandTruck()
  ...
  loadModel(GetHashKey(Config['Rob']['PDTruck']))
  Tacotruck = CreateVehicle(GetHashKey(Config['Rob']['PDTruck']), Config['Rob']['TruckSpawnCfg'], Config['Rob']['TruckSpawnHeadingCfg'], true, true)
  ...
  TriggerServerEvent('pp-weedheist:server:policeAlert', Config['Rob']['TruckSpawnCfg'])
  CheckInV = true
  CheckInVehicle()
  Delivery()
    ...
    if(distance < 6) then
      ...
      if IsControlJustPressed(0, 38) then
        if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
          ...
          DeletePed(guardPed)
          TriggerServerEvent('pp-weedheist:server:rewardItem', deliveryLocation.sellprice, deliveryLocation.item, deliveryLocation.itemamount)
...
RegisterNetEvent('pp-weedheist:client:policeAlert')
AddEventHandler('pp-weedheist:client:policeAlert', function(targetCoords)
    local P_0 = 250
    local P_1 = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 50.0)

    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)

    TriggerServerEvent('police:server:policeAlert', 'Gangster War Robbery in progress.')
