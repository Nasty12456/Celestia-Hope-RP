...
addMod = function(vehicle, modId, stock, modPrice)
    ...
    elseif modId == "popcorn" then
        ...
        if Config.Settings.enablePopcorn then
...
RegisterNUICallback("addShopItem", function(data, cbj)
    ...
    elseif data.modId == "neon" then
        if data.mod == 4 then
            ...
            cartMods["neon_0"] = data.mod
        else
            SetVehicleNeonLightEnabled(vehicle, data.mod, true)
            -- Neon için benzersiz anahtar oluştur
            local neonKey = "neon_" .. tostring(data.mod)
            cartMods[neonKey] = true
        end
    elseif data.modId == "livery" and useCountLivery then
        ...
        -- Neon dışındaki modlar için
        if data.modId ~= "neon" then
...
RegisterNUICallback("buyBasket", function(data,cb)
    ...
    TriggerCallback("buyBasketData", function(result)
        ...
        if retval then
            ...
            setVehicleProperties(vehicle, currentMods)
            SetVehicleWheelType(vehicle, currentWheelType)
            -- -- print(json.encode(cartMods, {indent = true}))
            for modId, mod in pairs(cartMods) do
                -- -- print(modId)
                if modId == 18 then
                    ToggleVehicleMod(vehicle, 18, (not IsToggleModOn(vehicle, 18)))
                elseif modId == 22 then
                    ToggleVehicleMod(vehicle, 22, (not IsToggleModOn(vehicle, 22)))
                elseif string.sub(modId, 1, 4) == "neon" or string.find(modId, "neon") then
                    if mod == 4 then
                        for i = 0, 3 do
                            SetVehicleNeonLightEnabled(vehicle, i, false)
                        end
                    else
                        local neonIndex = tonumber(string.match(modId, "%d+"))
                        if neonIndex then
...
RegisterNUICallback("closeBossMenu", function(data)
                            ...
                            if mData.job ~= "none" then
                                if PlayerData.job and PlayerData.job.name == mData.job and not menuOpened then
                                    ...
                                    if IsControlJustPressed(0, 38) and not menuOpened then
                                        ...
                                        openMechanicMenu(true, GetVehiclePedIsUsing(playerPed), mData.enableSociety)
                                    end
                                end
                            else
...
RegisterNUICallback("fireEmployee", function(data,cb)
    ...
    if src then
        -- TriggerServerEvent("0r-mechanic:server:fireEmployee", src)
        local p = promise.new() 
        TriggerCallback("0r-mechanic:fireEmployee", function(result)
            p:resolve(result)
        end, src)
...
RegisterNUICallback("moneyAction", function(data,cb)
    local type = data.type
    local amount = tonumber(data.amount)
    if type and amount then
        if amount > 0 then
            local p = promise.new()
            TriggerCallback("0r-mechanic:moneyAction", function(result)
                p:resolve(result)
            end, {
                type = type,
                amount = amount,
                index = currentBossMenuIndex
            })
            local result = Citizen.Await(p)
            cb(result)
        end
...
RegisterNUICallback("hireEmployee", function(data,cb)
    ...
    if src then
        local p = promise.new() 
        TriggerCallback("0r-mechanic:hireEmployee", function(result)
            p:resolve(result)
        end, src)
        local result = Citizen.Await(p)
        cb(result)
