...
Citizen.CreateThread(function()
    for mName, mData in pairs(Config.Locations) do
        if mData.showBlip then
            ...
            AddTextComponentString(mData.blipName)
            EndTextCommandSetBlipName(mData.blipHandle)
        end
    end
end)

openBossMenu = function(mechanicIndex)
    if Config.Locations[mechanicIndex].job == "unemployed" then return ShowNotification(Config.Locale["cant_open_boss_menu"]) end
    currentBossMenuIndex = mechanicIndex
    SetNuiFocus(true, true)
    
    -- Create a promise
    local p = promise.new()
    
    -- Trigger the callback and resolve the promise
    TriggerCallback("0r-mechanic:getBossMenuData", function(data)
        p:resolve({data = data.data, employees = data.players, activePlayers = data.activePlayers, money = data.playerMoneyInPocket})
    end, mechanicIndex)
    local result = Citizen.Await(p)
    local retval = result.data
    local employees = result.employees
    local activePlayers = result.activePlayers
