...
Citizen.CreateThread(function()
    GetSocietyMoney = function(job, price)
        if Config.Framework == "esx" then
            return exports['esx_addonaccount']:GetSharedAccount(('society_%s'):format(job))
        else
            return exports['qb-management']:RemoveMoney(job, price)
        end
    end
