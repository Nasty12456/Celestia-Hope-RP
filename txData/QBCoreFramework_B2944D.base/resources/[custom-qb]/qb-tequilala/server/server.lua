...
CreateThread(function()
    ...
    for _, v in pairs(cocktails) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-tequilala:client:DrinkAlcohol', source, item.name) end) end

	local beers = { "ambeer", "dusche", "logger", "pisswasser", "pisswasser2", "pisswasser3" }
    for _, v in pairs(beers) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-tequilala:client:DrinkAlcohol', source, item.name) end) end

	local drinks = { "sprunk", "sprunklight", "ecola", "ecolalight", "cranberry" }
    for _, v in pairs(drinks) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-tequilala:client:Drink', source, item.name) end) end

	local food = { "chocolate", "crisps" }
    for k, v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-tequilala:client:Eat', source, item.name) end) end
end)

---Crafting
RegisterServerEvent('qb-tequilala:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
	...
	if craftable then
		...
		for k, v in pairs(craftable[ItemMake]) do TriggerEvent("qb-tequilala:server:toggleItem", false, tostring(k), v, src) end
	end
	--This should give the item, while the rest removes the requirements
	TriggerEvent("qb-tequilala:server:toggleItem", true, ItemMake, amount, src)
end)

RegisterNetEvent("qb-tequilala:server:DupeWarn", function(item, newsrc)
...
RegisterNetEvent('qb-tequilala:server:playMusic', function(song, zoneNum)
    local src = source
    ...
    TriggerClientEvent('qb-tequilala:client:playMusic', src, { zone = zoneNum })
end)
RegisterNetEvent('qb-tequilala:server:stopMusic', function(data)
    local src = source
    ...
    TriggerClientEvent('qb-tequilala:client:playMusic', src, { zone = data.zoneNum })
end)
RegisterNetEvent('qb-tequilala:server:pauseMusic', function(data)
    local src = source
	local zoneLabel = Config.Locations[data.zoneNum].job..data.zoneNum
	if Config.Locations[data.zoneNum].Booth.playing then
        Config.Locations[data.zoneNum].Booth.playing = false
...
RegisterNetEvent('qb-tequilala:server:resumeMusic', function(data)
    local src = source
	local zoneLabel = Config.Locations[data.zoneNum].job..data.zoneNum
...
RegisterNetEvent('qb-tequilala:server:changeVolume', function(volume, zoneNum)
    local src = source
	local Booth = Config.Locations[zoneNum].Booth
	local zoneLabel = Config.Locations[zoneNum].job..zoneNum
...
RegisterNetEvent('qb-tequilala:server:toggleItem', function(give, item, amount, newsrc)
	local src = newsrc or source
	if give == 0 or give == false then
		...
		else TriggerEvent("qb-tequilala:server:DupeWarn", item, src) end -- if not boot the player
	else
