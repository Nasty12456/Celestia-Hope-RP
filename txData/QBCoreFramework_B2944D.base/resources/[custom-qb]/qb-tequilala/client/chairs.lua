...
CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		...
		exports['qb-target']:AddBoxZone("TeqChair"..k, v.coords.xyz, 0.7, 0.7, { name="TeqChair"..k, heading = v.coords.w, debugPoly=Config.Debug, minZ = v.coords.z-1.2, maxZ = v.coords.z+0.1, },
			{ options = { { event = "qb-tequilala:Chair", icon = "fas fa-chair", label = Loc[Config.Lan].targetinfo["sit_down"], loc = v.coords, stand = v.stand }, },
				distance = 2.2
		})
	end
end)

RegisterNetEvent('qb-tequilala:Chair', function(data)
	local canSit = true
	local sitting = false
