...
local function spawnObj(model, coords, heading)
    ...
    exports['qb-target']:AddTargetEntity(object, {
        options = { {
             ...
             label = "Craft",
             action = function()
                TriggerServerEvent("qb-crafting_sv:getWorkBenchData")
             end
        }
        },
        distance = 1.5
...
RegisterNUICallback("discardBlueprint", function(data, cb)
    if currentBenchId then
        QBCore.Functions.TriggerCallback('qb-crafting_sv:discardBlueprint', function (result)
            cb(result)
...
RegisterNUICallback("attemptCraft", function(data, cb)
    if currentBenchId then
        TriggerServerEvent("qb-crafting_sv:attemptCraft", currentBenchId, data.currentRecipe, data.amt, data.isBlueprintRecipe)
    end
end)
...
RegisterNetEvent("qb-crafting_cl:openCraftingBench", function(craftingBenchData, benchId)
...
RegisterNetEvent("qb-crafting_cl:increasedRep", function(craftingRep, attachmentRep)
