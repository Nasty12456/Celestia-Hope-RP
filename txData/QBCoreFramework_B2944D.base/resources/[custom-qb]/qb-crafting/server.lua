...
RegisterNetEvent("qb-crafting_sv:getWorkBenchData", function()
    local src = source
    local closestBench = getClosestBench(src)
    if closestBench then
        TriggerClientEvent("qb-crafting_cl:openCraftingBench", src, craftingBenches[closestBench], closestBench)
    else
        TriggerClientEvent('QBCore:Notify', src, 'Unable to find crafting bench', 'error')
    end
end)

RegisterNetEvent("qb-crafting_sv:getCraftingBenchBlueprints", function(benchId)
    local src = source
    if craftingBenches[benchId] then
        TriggerClientEvent("qb-crafting_cl:loadCraftingBlueprints", src, craftingBenches[benchId].blueprints)
    end
end)

QBCore.Functions.CreateCallback('qb-crafting_sv:discardBlueprint', function(source, cb, benchId, blueprintToRemove)
    local src = source
    if craftingBenches[benchId] then
...
RegisterNetEvent("qb-crafting_sv:attemptCraft", function(benchId, itemToCraft, amount, isBlueprintRecipe)
    local src = source
        ...
        if amount ~= "max" then
            ...
            if Player.Functions.AddItem(itemToCraft, amount) then
                ...
                if isAttachment then
                    ...
                    TriggerClientEvent("qb-crafting_cl:increasedRep", src, rep, attachmentRep + (points * amount))
                else
                    Player.Functions.SetMetaData("craftingrep", rep + (points * amount))
                    TriggerClientEvent("qb-crafting_cl:increasedRep", src, rep + (points * amount), attachmentRep)
                end
            ...
            if Player.Functions.AddItem(itemToCraft, maxCraft) then
                ...
                if isAttachment then
                    ...
                    TriggerClientEvent("qb-crafting_cl:increasedRep", src, rep, attachmentRep + (points * maxCraft))
                else
                    Player.Functions.SetMetaData("craftingrep", rep + (points * maxCraft))
                    TriggerClientEvent("qb-crafting_cl:increasedRep", src, rep + (points * maxCraft), attachmentRep)
                end
