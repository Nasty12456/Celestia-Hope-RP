...
if not Config.JimConsumables then
	CreateThread(function()
		...
		for k,v in pairs(drinks) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-beanmachine:client:Consume', source, item.name, "drink") end) end

		local food = { "cheesecake", "watermelon", "beandonut", "chocolate", "crisps", }
		for k,v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-beanmachine:client:Consume', source, item.name, "food") end) end
	end)
else
	local foodTable = {
...
---Crafting
RegisterServerEvent('qb-beanmachine:Crafting:GetItem', function(ItemMake, craftable)
	local src = source
	...
	if craftable then
		...
		for k, v in pairs(craftable[ItemMake]) do TriggerEvent("qb-beanmachine:server:toggleItem", false, tostring(k), v, src) end
	end
	--This should give the item, while the rest removes the requirements
	TriggerEvent("qb-beanmachine:server:toggleItem", true, ItemMake, amount, src)
end)
...
RegisterNetEvent('qb-beanmachine:server:toggleItem', function(give, item, amount, newsrc)
