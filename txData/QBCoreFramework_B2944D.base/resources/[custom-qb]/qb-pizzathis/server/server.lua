...
if not Config.JimConsumables then
	CreateThread(function()
		...
		for _, v in pairs(wines) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-pizzathis:client:DrinkAlcohol', source, item.name) end) end

		local beers = { "ambeer", "dusche", "logger", "pisswasser", "pisswasser2", "pisswasser3" }
		for _, v in pairs(beers) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-pizzathis:client:DrinkAlcohol', source, item.name) end) end

		local drink = { "sprunk", "sprunklight", "ecola", "ecolalight" }
		for _, v in pairs(drink) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-pizzathis:client:Drink', source, item.name) end) end
		...
		for _, v in pairs(food) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('qb-pizzathis:client:Eat', source, item.name) end) end
	end)
end
CreateThread(function()
	local boxes = { "capricciosabox", "diavolabox", "marinarabox", "margheritabox", "prosciuttiobox", "vegetarianabox" }
	for _, v in pairs(boxes) do QBCore.Functions.CreateUseableItem(v, function(source, item) TriggerEvent('qb-pizzathis:OpenBox', source, item.name) end) end
end)
