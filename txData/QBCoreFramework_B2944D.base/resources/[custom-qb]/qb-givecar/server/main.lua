...
RegisterNetEvent('qb-givecar:server:RequestCommands', function()
    local src = source
    if isAllowedToGiveCar(src) then
        TriggerClientEvent('qb-givecar:client:RequestCommands', src, true)
    end
end)
    ...
    if Config.CommandEnabled then
        ...
        if id and model and IsPlayerAceAllowed(src, Config.AceGroup) then
            ...
            if Config.TextEnabled then
                TriggerClientEvent('qb-givecar:client:sendText', src, Lang:t('text_message.sender'), Lang:t('text_message.sender_subject'), Lang:t('text_message.sender_message', { model = model, plate = plate, recipient = GetPlayerName(id) }))
                TriggerClientEvent('qb-givecar:client:sendText', id, Lang:t('text_message.sender'), Lang:t('text_message.recipient_subject'), Lang:t('text_message.recipient_message', { model = model, plate = plate, garage = Config.GarageName }))
            else
